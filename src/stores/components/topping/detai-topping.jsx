...
import HeaderHS2 from '../header/headerHSmini/headerHS2';
...
const DetailTopping = () => {
  ...
  const toppingId = location.state?.toppingId;
  
  useEffect(() => {
    if (toppingId) {
      console.log("Topping ID inside useEffect:", toppingId);
  
      // Fetch reviews from API
      const fetchReviews = async () => {
        try {
          const response = await axios.get(`https://be-order-food.vercel.app/api/rating/topping/${toppingId}`);
      ...
      const fetchToppingDetails = async () => {
        try {
          const response = await axios.get(`https://be-order-food.vercel.app/api/topping/get-topping/${toppingId}`);
          console.log("Topping details response:", response.data.data);
          setTopping(response.data.data);
        } catch (error) {
          console.error('Error fetching topping details:', error);
          toast.error('Failed to fetch topping details.');
        }
      };
    ...
    <div className='bg-[#F5F5F5]'>
        ...
        <div className="max-w-[1200px] mx-auto p-4 relative mt-10">
            ...
            <div className="bg-gray-200 relative w-full mb-4 md:mb-0 md:w-1/2">
                ...
                <img className='rounded-sm' src={topping?.Food_picture || ''} alt="" />
            </div>
            <div className="md:ml-4 md:w-1/2">
                <h1 className="text-2xl font-semibold mb-2">{topping?.Food_name || ''}</h1>
                <p className="text-gray-600 mb-2">Mô tả: {topping?.Food_detail || ''}</p>
                <p className="text-xl text-[#ef4b2c] font-semibold mb-4">Giá: {topping?.Price.toLocaleString() || ''} VND</p>
                <div>
                    Trạng thái: <span className='text-[#ef4b2c]'>{topping?.Food_status || ''}</span>
                </div>
                {/* <div>
